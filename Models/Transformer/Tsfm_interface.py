...
if config.dataset_type == 'classification':
    ...
    drop = 0.0
model_mol = TrfmSeq2seq(len(NewDic), 256, len(NewDic), 4, dropout=drop)
model_mol.load_state_dict(torch.load(config.tsfm_fp_weight_path, map_location='cuda'))

model_atom = TrfmSeq2seq(len(Token2Idx), 256, len(Token2Idx), 4, dropout=drop)
model_atom.load_state_dict(torch.load(config.tsfm_atom_weight_path, map_location='cuda'))
