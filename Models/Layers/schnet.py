...
class update_v(torch.nn.Module):
    ...
    def forward(self, v, e, edge_index):
        _, i = edge_index
        # print(f'e.shape = {e.shape} i.shape = {i.shape}')
        out = scatter(e, i, dim=0)
        # print(f'v.shape = {v.shape} out.shape = {out.shape}')
        out = self.lin1(out)
        out = self.act(out)
        out = self.lin2(out)
        return v + out
...
class SchNet(torch.nn.Module):
    ...
    def forward(self, batch_data):
        ...
        # pos = torch.from_numpy(pos).to(config.device)

        # print(f'z = {z.shape} pos = {pos.shape} batch = {batch.shape}')
        # print('========================================================')
        # print(f'pos: {len(pos)} batch: {len(batch)} z: {len(z)}')
        # print('========================================================')
